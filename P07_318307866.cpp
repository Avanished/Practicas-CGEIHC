...
int main()
{
	...
	// luz puntual blanca de la lampara
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// linterna ligada a la cámara de tipo flash
		...
		// faro azul ligado al coche
		glm::vec3 posFaroCoche = glm::vec3(2.0f + mainWindow.getmuevex(), 0.5f, -0.5f);
		glm::vec3 dirFaroCoche = glm::vec3(1.0f, -0.2f, 0.0f);
		spotLights[2].SetFlash(posFaroCoche, dirFaroCoche);

		// helicoptero en eje X controlado por teclas H y J
		GLfloat heliX = 0.0f + mainWindow.getmuevehelicoptero();

		// spotlight amarillo ligado al helicoptero
		...
		// coche con jerarquía
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-2.0f + mainWindow.getmuevex(), -0.2f, 1.0f));
